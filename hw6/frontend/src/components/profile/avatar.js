...
const Avatar = ({ username, email, dob, zipcode, img, updated, avatars, disabled, update, enableUpdateBtn }) => {
    if (!updated){
        img = avatars[username];
    }
	return (
        <div className="col-sm-4">
            ...
            <label className="control-label col-sm-12"><span id ="displayName">{username}</span></label>
...
const mapStateToProps = state => ({ username: state.logIn.username, email: state.profile.email, dob: state.profile.dob, 
                                    ...
                                    disabled: state.profile.avatarUpdateBtnDis });
